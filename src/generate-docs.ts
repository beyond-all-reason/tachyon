...
export async function generateServiceMarkdown<T extends Record<string, TObject>>(tachyonSchema: Record<string, Record<string, EndpointConfig>>, endpoints: T, serviceId: string): Promise<string> {
...
export async function generateEndpointMarkdown<T extends TObject>(schema: T, endpointConfig: EndpointConfig, serviceId: string, endpointId: string): Promise<string> {
    ...
    if ("request" in schema.properties) {
        markdown += await generateCommandMarkdown(schema.properties.request as TObject, serviceId, endpointId, "request");
    }

    if ("response" in schema.properties) {
        markdown += await generateCommandMarkdown(schema.properties.response as TUnion, serviceId, endpointId, "response");
    }

    return markdown;
}

export async function generateCommandMarkdown<C extends "request" | "response", T extends C extends "request" ? TObject : TUnion>(
    schema: T,
    serviceId: string,
    endpointId: string,
    commandType: C
